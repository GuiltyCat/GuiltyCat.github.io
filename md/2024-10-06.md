...
最初の引数はその次に取る引数(ptr)の指し示す先の型を表している．
LLVMでは全てのポインタは`void*`のように`ptr`であるため，`ptr`はどの型の値を指し示しているのかを明治する必要がある．

二番目の引数は先程言った通りにポインタ．

そして残りの引数は全てインデックスを示しているが，最初と二番目が何かによって意味が変化する．
ここから三番目以降のインデックスについて説明する．
...
そして何故後ろに複数のインデックスが並びうるのかというと，次のLLVM IR公式のサンプルを例に見てみる．
...
ここで`foo`関数に渡される`struct ST *s`は配列だと考えて欲しい．
そして`&s[1].Z.B[5][13]`のような階層を辿って要素にアクセスするには，
次のようにインデックスを書き連ねていく必要がある．
...
では`i64 1, i32 2, i32 1, i64 5, i64 13`を順番に辿っていく．

1. `i64 1`は`s[1]`の`1`を指している．
    - ここで`s`は`struct ST`の配列だから`i32`でなくてよい．
2. `i32 2`は`struct ST`における3番目の要素である`struct RT Z`を指している．
    - ここで`Z`は構造体の3番目の要素だから`i32`かつ定数でないといけない
    - `X`が0番目で`Y`が1番目，`Z`が3番目になる．
    - 構造体の型宣言の`{ i32, double, %struct.RT }`の何番目の要素かだと思うと分かりやすい．
    - 配列のインデックスのように等間隔にジャンプする訳ではない．なので`int`や`double`といった異なる幅のものであってもきちんと辿れる．
...
ややこしいのは，C言語の配列の要素を示すインデックスもあれば，構造体の要素の番地を示すインデックスもあることだ．
通常Cの配列の要素の場合，インデックスはポインタへの足し算で実現されるため，メモリの番地そのものと対応付けて考えることができる．
ところがLLVMの構造体の要素の番地を示す場合は，メモリの番地は単純な足し算ではなくなってしまう．

さあ，次からがさらにややこしくなる．

次にLLVMの公式ページより次の例を見てみる．
...
まず，最初に抑えておかないといけないことは，LLVM IRにおいて配列とポインタは明確に区別されており，別物であるということだ．

つまり`getelementptr i32, ptr %1`と`getelementptr [10 x i32], ptr %1`における`%1`は全くの別物として扱われる．

`i32, ptr %1`の方はあくまで`i32`へのポインタとして扱われるため，その後に続く引数は1つで，アクセスする要素のインデックスを指定するだけでよい．
しかし，`[10 x i32], ptr %1`の場合，`[10 x i32]`という一つの型へのポインタとして解釈される．
つまり，`getelementptr [10 x i32], ptr %a, i32 1`とした場合，`int a[10]; a[1]`のように解釈されるのではなく，
`int* a[10]; a[1]`として解釈されるイメージだ．
`a[10]`が一つの要素として見做されるため．`1`と指定すると，数のように解釈されるのではなく，
...
これが分かれば，なぜ引数が1つのものと2つのものがあるのか，同じような配列へのアクセスなのに何故変化するのかの意味が分かってくる．

`%t1`の`i32 1`は`%struct.ST`というポインタの`s[1]`へのアクセスなので1つでよい．
...
つまり，`%t1`は`struct.ST`という単位で見た時の1番目の要素を示している．
`%t1`を`struct.ST`という単位で見た時に，`0`番目の要素は`s[1]`自身であり，その中のさらに2番目の要素を`%t2`に入れている．

後もうちょっとだが今日はここまで．おやすみなさい．
