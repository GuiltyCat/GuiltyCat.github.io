...
日々使うのはこの二つのコマンドだけだ．
また，過去の記事を更新する場合にも便利な機能が`aritcle.bash`にはあるが，それについては記事の下の方か`bash article.bash -h`を参照されたい．

`article.bash` creates today's article Markdown file and open  by `nvim`.
...
基本は`article.bash`を使って自動生成させている．
...
Each article file is generated by `article.bash`.
...
article.bash, upload.bash
---------------

`article.bash`は日々の記事を作るのに使える．
また，過去の記事の一覧表示や，修正が簡単にできるようにいくつかのオプションがある．
詳細は`--help`か`-h`でこのコマンドを起動する分かる．
...
`article.bash` has serveral options.

- `--help` or `-h` option.
	You can see help of `article.bash`.
- `--list` or `-l` option. 
